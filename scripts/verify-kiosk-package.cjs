...
const sdkDll = path.join(resources, "hardware/_internal/bin", sdkName)
assert.ok(fs.existsSync(sdkDll), "Installer must include the Bixolon SDK: " + sdkName)
assert.equal(peArchitecture(sdkDll), arch, "Bixolon SDK payload architecture")
