...
const { APP_ID, ID, VERSION, check, sign, releaseOf, requestOf, newer } = require("../electron/update-protocol")
const { values: args, positionals } = parseArgs({
  allowPositionals: true,
  options: Object.fromEntries(["out", "key", "device", "property", "name", "file", "version", "from-version"].map((key) => [key, { type: "string" }])),
})
const releasePath = (version) => "releases/v" + version.replaceAll(".", "_")
async function main() {
  ...
  if (action === "publish") {
    ...
    const key = releasePath(args.version), previous = await store.get(key)
    check(!previous.value, "This release version is immutable")
    const signed = await publishInstaller(config, args.file, args.version, fs.readFileSync(args.key))
  ...
  if (action === "register") {
    ...
    try {
      await store.put(key, { kind: "kiosk", property: args.property, name: args.name || args.device, revoked: false, code }, device.generation)
      ...
      fs.writeFileSync(fd, JSON.stringify({ projectId: config.projectId, code, deviceId: args.device, property: args.property, expiresAt }))
  ...
  if (action === "status") {
    ...
    console.log(JSON.stringify({ deviceId: args.device, property: device.value.property, registered: !!binding, revoked: device.value.revoked,
  ...
  const signedRelease = (await store.get(releasePath(args.version))).value
  const release = releaseOf(signedRelease, publicKey)
  ...
  const request = { appId: APP_ID, action: "install", id: crypto.randomUUID(), deviceId: args.device,
    version: release.version, fromVersion: args["from-version"], createdAt: Date.now(), expiresAt: Date.now() + 24 * 3600000, ...ticket }
