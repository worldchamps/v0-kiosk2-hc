...
### 2026-09-12 예약표 저장 전 실패 후 재시도 복구 (1.3.5)

기존 `saving` 기록은 예약표 L/M이 모두 비어 있어도 무조건 대기해, 앱 업데이트 후에도
같은 예약의 체크인 결과 확인이 끝나지 않을 수 있었다. 최초 기록이 비어 있는 원인
(저장 실패, 응답 유실 또는 이후 수정)은 기존 데이터만으로 단정하지 않는다.

1.3.5는 같은 예약·객실·일정의 `saving` 작업에서 L/M이 모두 비어 있고 마지막 저장 시도 후
60초가 지났을 때만 원래 입실 시각으로 동일한 셀 값을 재시도한다. 동시 요청은 Firebase의
조건부 쓰기 임대로 묶으며 새 예약을 만들거나 기존 작업을 삭제하지 않는다.
쓰기 직전 같은 행의 예약 ID·성함·숙소·객실·입퇴실 일정을 다시 읽는다. 행 이동/수정,
부분 입실 기록, 다른 입실 시각 또는 취소 상태는 덮어쓰지 않는다.
이미 동일한 L/M이 있으면 추가 저장 없이 기존 완료 절차로 진행한다.

예약표 쓰기 실패는 인증/수정 권한, 행 변경, 응답 미확정 등 제한된 사유로 안내한다.
외부 API의 오류 원문·URL·인증정보는 작업 기록에 저장하지 않는다.
객실 명령은 기존 단일 상태 전환과 원자적 DB 갱신을 통해 한 번만 발행한다.
`committing` 결과가 불명확한 작업은 자동 재발행하지 않으며, 다른 예약 기록은 건드리지 않는다.
현장 복구 목적으로 예약을 임의 삭제하거나 완료로 바꾸지 않는다.

### 2026-09-12 토스 연결과 업데이트 대기 분리 (1.3.4)

토스 프론트 WebSocket 접속이 거절되거나 인증이 완료되지 않아도, 거래 없는 첫 화면에서
소프트웨어 업데이트는 진행할 수 있도록 단말기 인증 조건만 분리했다.
이는 토스 결제 연동이나 플러그인을 수정·배포한 것이 아니며, 카드 결제에는 기존 인증이 필요하다.
60초 무입력, 신선한 화면 상태, 미확정 결제·체크인 보호, 활성 IPC/HTTP 종료,
소유한 장비 프로그램 연결 및 진행 중 토스 요청 없음, 화면 전환 확인은 유지한다.
설치 대기의 이유는 장비 상태의 message로 보고한다. 사유 문자열을 설치 허용으로 오인하지 않도록
prepare가 명시적으로 true를 반환할 때만 설치한다. 대기 사유 보고의 일시적 네트워크 장애는 재시도를 막지 않는다.
운영자가 취소한 대기 요청은 online으로 정리하며, 이를 설치 완료로 보고하지 않는다.

기존 1.3.2/1.3.3에서 토스 미인증 때문에 설치가 막힌 PC는 서버 요청만 바꿔서는 새 코드를 적용할 수 없다.
해당 PC의 대기 요청과 실제 거래 없음 상태를 확인한 뒤, 기존 요청을 취소하고 같은 비트수의
1.3.4 설치파일을 기존 설치 위치에 한 번 적용한다. 기존 프로그램 제거, 재등록, 사용자 데이터 삭제는 하지 않는다.
설치 후 서버 버전과 실제 첫 화면을 확인한다. 새 등록창이 나오면 설정을 새로 입력하지 말고 중단한다.
수정본은 1.3.3의 체크인 캐시 오류 수정도 포함한다. 실제 단말기 카드 승인이나 현장 자동 설치 성공은 별도 검증이다.
