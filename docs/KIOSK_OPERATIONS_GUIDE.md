...
토스 프론트의 시리얼 연결은 키오스크 쪽 COM 포트·통신 속도와 프론트 플러그인 쪽 설정을
각각 일치시켜야 합니다. 페어링 키는 읽기 화면에 표시되지 않으며, 변경할 때만 새 값을 입력합니다.
한 COM 포트에 여러 장비를 지정할 수 없습니다. SAM4S의 COM 포트는 앱 화면이 아니라
Windows 프린터 속성에서 지정합니다.

저장된 값은 장비 등록 때 만든 암호화 설정에만 반영됩니다. 실행 중인 장비 연결은 즉시
변경되지 않으므로, 결제·예약 처리가 없는 때 앱을 정상 종료하고 다시 실행한 후 관리자 모드의
**기기 연결 상태**와 테스트 출력을 확인합니다. 최초 등록 뒤에는 `.env.local`을 편집하거나
새 설치파일을 덮어 설치해도 이 암호화 설정이 자동으로 바뀌지 않습니다. 이 화면을 사용하려면
화면 기능이 포함된 새 설치형 앱 버전으로 먼저 업데이트해야 합니다.
...
2. PMS가 Firebase `kiosk_sales_config`에 저장한 판매 여부, 판매 시간, 숙박·대실 현금·카드 요금을 적용합니다.
...
현장예약의 입·퇴실 시각은 `Reservations` 시트 H/I에 `YY.MM.DD/HH:MM` 형식으로
기록합니다. 입실 시각은 결제 완료 시각이고, 대실 이용 시간과 숙박 퇴실 시각은 PMS의
키오스크 판매 설정을 따릅니다.

PMS의 객실별 판매 유형은 `숙박`, `대실`, `숙박/대실`, `판매불가`입니다. 허용된
유형 중 0원보다 큰 결제 요금이 있는 유형만 노출하며, 기존 숙소·결제수단 제한은 유지합니다.
PMS에서 숙소별 대실 판매 마감·이용시간과 **현장판매 숙박 퇴실시간(기본 11:00)**을
변경할 수 있습니다. 시간 정책 변경은 이후 새로 만드는 현장예약에 적용됩니다.
다른 플랫폼·PMS 예약 및 이미 생성된 현장예약은 `Reservations` I열에 기록된 개별
퇴실 날짜·시간을 그대로 조회·표시·출력합니다. 기록에 시간이 없을 때만 11:00으로 보완하며,
현재 현장판매 정책으로 기존 예약의 퇴실시간을 덮어쓰지 않습니다.

`kiosk_sales_config`가 아직 없는 숙소는 호환성을 위해 기존 `pms_status` 요금과 기본 시간
정책을 사용합니다. 기본값은 대실 180분, 숙박 퇴실 11:00이며 property1·property3의
대실은 00:00~21:00, property2·property4의 대실은 종일 허용입니다.

PMS 설정으로 판매를 허용해도 객실이 실제 `공실`이어야 하며, 현장 프로그램이 객실을
판매 불가로 표시했거나 자판기 판매를 막은 객실은 키오스크에 나타나지 않습니다. 결제 직전
서버가 상태·판매시간·설정요금을 다시 확인하므로 화면을 오래 열어 둔 경우에도 최신 설정이
최종 적용됩니다.
...
설치형 1.3.10부터 SDK를 `resources/hardware/_internal/bin`에 반드시 포함하고,
배포 검증에서 해당 비트수의 DLL을 실제 로딩합니다. 1.3.9 설치파일에서는 SDK 누락이 확인됐습니다.
SDK가 없는 경우 Windows에 프린터가 표시되더라도 앱의 SDK 인쇄 경로는 동작하지 않을 수 있습니다.

현장 확인 순서:

1. 전원·용지·덮개를 확인하고 FEED 버튼이 있는 모델은 용지 이송을 확인합니다.
2. 관리자 모드의 `프린터 테스트` → `테스트 인쇄`에서 실제 종이와 절단을 확인합니다.
3. 인쇄 서버 연결이나 명령 전달 표시는 실제 프린터 출력 성공을 보장하지 않습니다.
4. SDK 포함 버전에서도 출력이 없으면 실제 COM 번호와 `PRINTER_PORT`, 통신 속도 `115200`,
   케이블과 다른 프로그램의 포트 점유를 확인합니다. COM2는 기본값이며 현장 번호로 단정하지 않습니다.
